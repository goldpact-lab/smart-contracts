...
contract Treasury is Ownable {
    ...
    event ReservesExchanged(address indexed fromToken, address indexed toToken, uint amount);
    ...
    function incurDebt(uint _amount, address _token) external {
        ...
        uint currentDebt = debtorBalance[msg.sender];

        // The total debt after this new borrowing
        uint newTotalDebt = currentDebt + value;

        // Get the user's sOHM balance
        uint userBalance = IERC20(sOHM).balanceOf(msg.sender);

        // Check the user's maximum available debt limit:
        // (balance + existing collateral) - current debt
        uint availableDebt = (userBalance + lockedCollateral) - currentDebt;
        require(value <= availableDebt, "Exceeds debt limit");

        // Ensure the user locks enough collateral to cover the *newTotalDebt*
        if (lockedCollateral < newTotalDebt) {
            uint additionalCollateral = newTotalDebt - lockedCollateral;

            // Pull additional collateral from the user
            IERC20(sOHM).transferFrom(
                msg.sender,
                address(this),
                additionalCollateral
            );
            debtorCollateral[msg.sender] += additionalCollateral;
        }

        // Update the user's debt to the new total
        debtorBalance[msg.sender] = newTotalDebt;

        // Update overall protocol debt and reserves
        totalDebt += value;
        totalReserves -= value;
    ...
    function swapReserve(
        ...
        emit ReservesExchanged(fromToken, toToken, _amount);
    }
    ...
    function swapUSDTToMAIN(
       ...
       emit ReservesExchanged(fromToken, toToken, amount);
   }
